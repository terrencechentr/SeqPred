...
# transformers 4.40.* 移除了 can_return_tuple，这里做兼容处理
try:
    from transformers.utils import can_return_tuple
except ImportError:  # pragma: no cover - 兼容缺省模块
    def can_return_tuple(func):
        return func

# Try to import optional utilities
try:
    from transformers.models.qwen2.modeling_qwen2 import BlockMask, make_flex_block_causal_mask
except ImportError:
    BlockMask = None
    make_flex_block_causal_mask = None

try:
    from transformers.models.qwen2.modeling_qwen2 import AttentionMaskConverter
except ImportError:
...
@dataclass
class CseptOutput(ModelOutput):
    """
    Base class for outputs of continuous sequence prediction models.

    Args:
        loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `labels` is provided) :
            Classification loss.
        preds (`torch.FloatTensor` of shape `(batch_size, sequence_length, config.num_labels)`):
...
class CseptModel(Qwen2PreTrainedModel):
    config_class = CseptConfig  # 指定配置类
    
    def __init__(self, config: CseptConfig):
    ...
    def forward(
        ...
        if input_embeds is None:
            inputs_embeds = self.embed_proj(input_values)
...
class CspetForSequencePrediction(Qwen2PreTrainedModel):
    config_class = CseptConfig  # 指定配置类
